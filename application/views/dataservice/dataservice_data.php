<head>
    <title>Data Service - Administrator POS Amnotel</title>
</head>

<div class="row d-flex align-items-center">
				<div class="mr-auto col-md-4 mt-1 mb-1">
					<h5 class="m-subheader__title">
                        <label for=""><span><a href="<?php echo base_url('dataservice'); ?>">Service</a> / Data Service</span></label>
					</h5>
				</div>
...
<!--<div class="row">
    <div class="col-lg-12">  
        <ol class="breadcrumb">
            <li><a href="<?php echo base_url('dataservice'); ?>">Service</a></li>
            <li class="active">Data Service</li>
            <div class="col-md-10 mt-1 mb-1 text-right">
		...
		<script>
            ...
            function timestamp() {
                $.ajax({
                    url: 'https://pos.amnotel.co.id/calendar/jam',
...
<div class="row">
    <div class="col-lg-12">
            <?php //echo anchor('dataservice/create', 'Add', array('class' => 'btn btn-primary btn-sm')); ?>        
        <div class="panel panel-default">
            ...
            <div class="panel-body"><!-- pencarian -->
                ...
                <table class="m-datatable" id="html_table" width="100%">
                                ...
                                <?php 
                                $no = 1;
                                foreach($transaksiteknisi->result() as $row) {
                                ?>
                                    <tr style="height: 56px;">
                                        <td class="text-center"><?php echo $no;?></td>                                        
                                        <td class="text-center"><?php echo $row->id_transaksi;?></td>
                                        <td class="text-center"><?php echo tgl_indonesia_transaksi($row->tanggal_service);?></td>
                                        <!--<td class="text-center"><?php echo tgl_indonesia_transaksi($row->tanggal_selesai);?></td>                                        -->
                                        <td class="text-center"><?php echo $row->merk;?></td>  
                                        <td class="text-center"><?php echo $row->imei;?></td>                                    
                                        <td class="text-center"><?php echo $row->garansi;?></td>
                                        <td class="text-center"><?php echo $row->kerusakan;?></td>
                                        <!--<td class="text-center"><?php echo $row->estimasi;?></td>
                                        <td class="text-right"><?php echo rupiah($row->biaya);?></td>-->                                                                              
                                        <td class="text-center"><?php echo $row->status_transaksi;?></td>
                                        <td class="text-center"><?php echo $row->full_name;?></td>
                                                                                
                                        <td class="text-center">
                                            <span style="width:113px">
                                                <a href="<?php echo base_url('dataservice/edit/'.$row->id_transaksi) ?>" name="edit" class="btn btn-success m-btn m-btn--icon btn-lg m-btn--icon-only">
                                                    <i class="fa fa-pencil"></i>
                                                </a>                                                
                                                <a href="#" name="<?php echo $row->imei;?>" class="hapus btn btn-danger m-btn m-btn--icon btn-lg m-btn--icon-only deletekar" kode="<?php echo $row->id_transaksi;?>">
                                                    <i class="fa fa-trash"></i>
                                                </a>
                                            </span>
                                        </td>
                                        <!--<td class="text-center">
                                            <a href="<?php echo base_url('dataservice/edit/'.$row->kode_transaksiteknisi) ?>"><input type="submit" class="btn btn-success btn-xs" name="edit" value="Edit"></a>
                                            <a href="#" name="<?php echo $row->judul;?>" class="hapus btn btn-danger btn-xs" kode="<?php echo $row->kode_dataservice;?>">Hapus</a>
                                        </td>-->
                                    </tr>
                                <?php $no++; } ?>    
                                </tbody>
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <input type="hidden" name="idhapus" id="idhapus">
                <p>Apakah anda yakin ingin menghapus dataservice dengan IMEI <strong class="text-konfirmasi"> </strong> ?</p>
        </div>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $("#konfirmasi").click(function(){
            ...
            $.ajax({
                url:"<?php echo site_url('dataservice/delete');?>",
                type:"POST",
                data:"id_transaksi="+id_transaksi,
                cache:false,
                success:function(html){
                    location.href="<?php echo site_url('dataservice/index/delete-success');?>";
                }
            });
        });
    });
